...
exports.fromWGS84 = function(location) {
  ...
  if (zone === 31 && latBand === 'V' && location.longitude>= 3) { zone++; λ0 += (6) * DEG_TO_RAD; }
  // adjust zone & central meridian for Svalbard
  if (zone === 32 && latBand === 'X' && location.longitude<  9) { zone--; λ0 -= (6) * DEG_TO_RAD; }
  if (zone === 32 && latBand === 'X' && location.longitude>= 9) { zone++; λ0 += (6) * DEG_TO_RAD; }
  if (zone === 34 && latBand === 'X' && location.longitude< 21) { zone--; λ0 -= (6) * DEG_TO_RAD; }
  if (zone === 34 && latBand === 'X' && location.longitude>=21) { zone++; λ0 += (6) * DEG_TO_RAD; }
  if (zone === 36 && latBand === 'X' && location.longitude< 33) { zone--; λ0 -= (6) * DEG_TO_RAD; }
  if (zone === 36 && latBand === 'X' && location.longitude>=33) { zone++; λ0 += (6) * DEG_TO_RAD; }

  var φ = location.latitude * DEG_TO_RAD;      // latitude ± from equator
  var λ = location.longitude * DEG_TO_RAD - λ0; // longitude ± from central meridian
...
exports.toWGS84 = function(utmCoords, zone) {
    ...
    var λ0 = ((z-1)*6 - 180 + 3) * DEG_TO_RAD; // longitude of central meridian
    λ += λ0; // move λ from zonal to global coordinates

    // round to reasonable precision
    var lat = Number((φ * RAD_TO_DEG).toFixed(11)); // nm precision (1nm = 10^-11°)
    var lon = Number((λ * RAD_TO_DEG).toFixed(11)); // (strictly lat rounding should be φ⋅cosφ!)
